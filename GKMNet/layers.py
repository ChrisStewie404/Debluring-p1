...
class CLSTM_cell(nn.Module):
    ...
    def init_hidden(self, batch_size, shape):
        if torch.cuda.is_available():
            ...
            return (torch.zeros(batch_size, self.num_features, shape[0], shape[1]).cpu(),
                torch.zeros(batch_size, self.num_features, shape[0], shape[1]).cpu())
    ...
    def __init__(self, num_kernels=21, max_kernel_size=21, mode='TG', channels=3):
        ...
        for i in range(num_kernels):
            ...
            if torch.cuda.is_available():
                ...
                weight[i + 1] = (F.pad(gen_gaussian_kernel(kernel_size, sigma=0.25 * (i + 1)),
                                   [pad, pad, pad, pad])).squeeze(0)
        ...
        if torch.cuda.is_available():
            ...
            weight[0] = (F.pad(torch.FloatTensor([[[[1.]]]]),
                           [pad, pad, pad, pad])).squeeze(0)
            kernel = np.repeat(weight, self.channels, axis=0)
