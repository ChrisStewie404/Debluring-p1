...
class GKMNet(nn.Module):
    ...
    def forward(self, input_blur_256, input_blur_128, input_blur_64):
        ...
        '''scale 2'''
        db128, h, c = self.forward_step(input_blur_128, F.interpolate(db64, scale_factor=2, mode='bilinear'), (h, c))
        h = F.interpolate(h, scale_factor=2, mode='bilinear')
        c = F.interpolate(c, scale_factor=2, mode='bilinear')
        '''scale 3'''
        db256, _, _ = self.forward_step(input_blur_256, F.interpolate(db128, scale_factor=2, mode='bilinear'), (h, c))
